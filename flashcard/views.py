...
def novo_flashcard(request):
    ...
    if request.method  == "GET":
        ...
        flashcards = Flashcard.objects.filter(user = request.user) # Puxando do banco os flashcards por usuário logado
        ...
        # Criando representação de flashcard em memória (não salva no bd)
        flashcard = Flashcard(
            user = request.user,
            pergunta = pergunta,
            resposta = resposta,
        ...
        # salva no banco os POSTS dos usuários
        flashcard.save()
...
def deletar_flashcard(request, id):
    # fazer a validação de segurança usando o request.user 
...
def iniciar_desafio(request):
    ...
    if request.method == "GET":
        ...
        categorias = request.POST.getlist('categoria') #Como o select no html é do tipo multiple, ele retorna uma lista
        dificuldade = request.POST.get('dificuldade')
        ...
        flashcards = (
                    ...
                    .order_by('?')  # A partir do orderby (?) podemos selecionar as recorrencias de forma aleatória
                    )
...
def relatorio(request, id):
    ...
    #implementar o ranking que é mostrado no figma
